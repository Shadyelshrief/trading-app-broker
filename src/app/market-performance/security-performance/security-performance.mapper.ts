...
export function mapSecurityRowsToChartData(rows: readonly SecurityPerformanceRow[]): ChartSeriesPoint[] {
  return rows.map((row) => ({
    ...
    value: row.closingPrice,
    volume: row.volumeTraded
  }));
}

export function buildSecurityChartSeries(
  label: string,
